...
- **所有 Hangfire Job 必须接入 `ITaskProgressService`**，让用户在全局任务面板（`TaskPanel.vue`）中实时看到任务进度。
- 实施步骤：
  1. 构造函数注入 `ITaskProgressService _taskProgress`
  2. 执行开始时调用 `var bgTaskId = await _taskProgress.StartAsync(userId, projectId, taskType, title)`
  3. 关键步骤调用 `await _taskProgress.ReportProgressAsync(bgTaskId, progress%, "步骤说明...")`
  4. 完成时调用 `await _taskProgress.CompleteAsync(bgTaskId, "完成消息")`
  5. catch 块中调用 `await _taskProgress.FailAsync(bgTaskId, ex.Message)`
- 新增任务类型时，必须在 `BackgroundTaskType`（`Domain/Enums/BackgroundTaskType.cs`）中先添加对应枚举值。
- 前端通过 SignalR `/hubs/task-progress` 接收推送，Hub 按 userId 分组，无需额外前端改动。
- 已接入示例：`ExtractNovelAssetsJob`、`ChapterDraftJob`、`NovelEndingSummaryJob`，参考这些文件的写法。
