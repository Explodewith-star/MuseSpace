...
public interface ICanonFactRepository
{
    ...
    Task<List<CanonFact>> GetByOutlineAsync(Guid projectId, Guid storyOutlineId, CancellationToken ct = default);
    Task<List<CanonFact>> GetActiveAsync(Guid projectId, CancellationToken ct = default);
    Task<List<CanonFact>> GetActiveByOutlineAsync(Guid projectId, Guid storyOutlineId, CancellationToken ct = default);
    Task<List<CanonFact>> GetLockedAsync(Guid projectId, CancellationToken ct = default);
    Task<List<CanonFact>> GetLockedByOutlineAsync(Guid projectId, Guid storyOutlineId, CancellationToken ct = default);
    ...
    Task<CanonFact?> GetByKeyAsync(Guid projectId, Guid storyOutlineId, string factType, string factKey, CancellationToken ct = default);
