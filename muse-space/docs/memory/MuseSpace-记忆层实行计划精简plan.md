...
> 技术栈：Vue3 + TypeScript + ASP.NET Core + PostgreSQL 16 + pgvector
> 文档版本：v3.0 | 最后更新：2026-04-19
...
| v2.0 | 2026-04-19 | 新增 Neo4j + GraphRAG 集成方案 |
| **v3.0** | **2026-04-19** | **回归务实路线：聚焦"原著导入 + 检索 + 续写/番外"；精简为 4 阶段；移除前期对 Redis、COS、Neo4j、SK、Worker 的强依赖；主线使用 ISkill + SkillOrchestrator** |

---

## 文档说明

本文档基于 MuseSpace **当前已跑通的生成链路**（`SkillOrchestrator`、`SceneDraftSkill`、`StoryContextBuilder`、Prompt 模板、OpenRouter 调用），为"导入原著小说 → 检索原著记忆 → 续写/写番外"这一核心场景，规划先跑通、后工业化的记忆层建设路径。

**核心原则：**
- 先跑通，再逐步替换工业化组件
- 主线继续使用 `ISkill + SkillOrchestrator`，不在前期引入 Semantic Kernel
- 每阶段可独立运行、可验收
- 不过度设计，不提前优化
- Redis、COS、Neo4j、独立 Worker、Multi-Agent 等均为后续扩展，前期不引入
...
```
阶段一：现有系统对接与最小底座补齐    → 复用已有 Skill 体系，仅补 PostgreSQL + pgvector + 本地文件存储
阶段二：原著导入与基础记忆构建        → 导入 TXT/Markdown 小说，切片，向量化，检索相关原著段落
阶段三：续写/番外场景的记忆增强       → 原著检索 + 已有 Character/WorldRule/StyleProfile/Chapter 双源融合注入 Prompt
阶段四（扩展）：工业化与高级能力      → 按需接入 Redis、COS、Neo4j、Semantic Kernel、Multi-Agent 等
```
...
- `ISkillOrchestrator` + `ISkill` 编排体系
- `SceneDraftSkill` 场景草稿生成
- `StoryContextBuilder` 上下文构建
- `prompts/` 文件系统 Prompt 模板
- OpenRouter LLM 调用（`OpenRouterLlmClient`）
- `Character`、`WorldRule`、`StyleProfile`、`Chapter` 等已有领域模型

## 存储层架构（前期精简）

```
PostgreSQL 16 + pgvector    → 业务数据 + 切片 + 向量检索
本地文件存储 dev-data/      → 原始小说文件 + 解析产物
```

> Redis、Neo4j、COS、独立 Worker 等均为阶段四扩展内容，前期不引入。
...
### 目标
在 MuseSpace 已有 Skill 编排体系基础上，仅补齐 PostgreSQL + pgvector + 本地文件存储，为原著导入和记忆检索提供最小底座。

**已有，无需重建：** `SkillOrchestrator`、`SceneDraftSkill`、`StoryContextBuilder`、Prompt 模板、OpenRouter 调用

**本阶段只补：** PostgreSQL + pgvector + 本地文件存储

**暂不引入：** Redis、COS/OSS/MinIO、独立 Worker、Neo4j
...
使用本地 `dev-data/files/` 目录，后续按需替换为 COS/OSS。
...
#### 细节注意
- 数据库只存文件逻辑 Key（如 `raw/doc-001.txt`），不存绝对路径
- 后端通过 `IStorageService` 接口操作，底层可替换
- 云对象存储（COS/OSS/MinIO）留到阶段四按需接入
...
### 步骤 1.5：在现有项目中补充记忆层模块

> 项目已有 `MuseSpace.Api`、`MuseSpace.Application`、`MuseSpace.Domain`、`MuseSpace.Infrastructure`、`MuseSpace.Contracts` 等结构，无需从零搭建。

#### 只需补充
- 在 `MuseSpace.Domain` 中新增记忆层领域模型（Novel、NovelChunk 等）
- 在 `MuseSpace.Infrastructure` 中新增记忆层数据访问
- 在 `MuseSpace.Application` 中新增原著导入相关 Skill / Service

#### 细节注意
- 不新增独立 Worker 项目，前期 embedding/chunking 在 API 进程内异步执行
- 不新增 Graph 模块，留到阶段四扩展
- 复用已有 `ISkillOrchestrator` 编排体系
...
### 目标
实现"导入 TXT/Markdown 小说 → 切片 → 向量化 → 检索相关原著段落"的完整流水线。本阶段的目标不是通用 RAG，而是为续写/番外提供"原著参考片段"。
...
### 阶段二验收标准
- [ ] 导入一个 TXT/Markdown 小说，系统能完成解析→切片→向量化
- [ ] 给定一段续写场景描述，能返回最相关的原著段落
...
### 目标
将阶段二的原著检索结果，与项目已有的 `Character`、`WorldRule`、`StyleProfile`、`Chapter` 等领域数据进行**双源融合**，一起注入 Prompt，服务续写/番外生成。主线继续使用 `ISkill + SkillOrchestrator`。
...
### 阶段三验收标准
- [ ] 原著检索结果能与已有 Character/WorldRule/StyleProfile/Chapter 双源融合
- [ ] 融合后的上下文能正确注入 Prompt 模板
- [ ] 通过已有 `SkillOrchestrator` 编排，端到端完成"导入原著 → 检索 → 续写生成"
- [ ] 风格规则能影响最终输出
- [ ] 记忆检索接口：给定续写场景，能返回原著相关段落 + 已有领域数据的混合结果
...
### 目标
在前三个阶段跑通核心链路后，按需引入工业化组件提升能力上限。以下能力均为**可选扩展**，根据实际需要逐项接入。

### 可选扩展清单

| 组件 | 用途 | 引入条件 |
|------|------|---------|
| Redis | 上下文缓冲、热点缓存 | 高频读写场景出现性能瓶颈 |
| COS/OSS | 云对象存储替换本地文件 | 多人协作或部署到云端 |
| Neo4j | 实体关系图谱、多跳推理 | 实体量大、需要图算法 |
| Semantic Kernel | 可选编排框架 | 需要更复杂的 Plugin 生态 |
| 独立 Worker | 后台任务独立部署 | embedding 任务阻塞 API |
| Multi-Agent | 多 Agent 协作 | 单 Agent 质量不足 |
| GraphRAG | 图遍历 + 向量 + 关键词三路融合 | 检索质量需进一步提升 |

> **主线编排仍建议继续使用 `ISkill + SkillOrchestrator`，SK 作为可选补充。**
...
- [ ] 按需接入的组件能正常运行
- [ ] 不影响已有的 Skill 编排链路
- [ ] 接入新组件后，续写/番外生成质量有可观测的提升

---

> 以下为阶段四中各可选扩展的详细参考方案，按需执行。
...
| 阶段 | 核心交付 |
|------|---------|
| **阶段一** | PostgreSQL + pgvector + 本地文件存储 + 现有项目补充记忆层模块 |
| **阶段二** | TXT/Markdown 小说导入、切片、向量化、原著段落检索、Hybrid RAG |
| **阶段三** | 原著检索 + Character/WorldRule/StyleProfile/Chapter 双源融合、续写/番外生成 |
| **阶段四（扩展）** | 按需接入：Redis、COS、Neo4j、Semantic Kernel、Worker、Multi-Agent、GraphRAG |
...
| 技术 | 引入时机 | 备注 |
|------|---------|------|
| PostgreSQL 16 + pgvector | 阶段一 | 底座，必选 |
| 本地文件存储 | 阶段一 | 过渡方案，后续可替换 COS/OSS |
| Hybrid RAG（向量 + 关键词） | 阶段二 | 推荐的检索方案 |
| ISkill + SkillOrchestrator | 已有 | **主线编排，持续使用** |
| Redis | 阶段四可选 | 高频读写场景按需 |
| Neo4j / GraphRAG | 阶段四可选 | 实体量大时按需 |
| Semantic Kernel | 阶段四可选 | 作为补充，不替代主线编排 |
| 本地大模型（Ollama） | 阶段四可选 | 降低 API 成本 |

---

*文档版本：v3.0 | 适用项目：MuseSpace | 最后更新：2026-04-19*
*v3.0 修订：回归务实路线，聚焦原著导入 + 续写/番外场景，精简为 4 阶段，主线使用 ISkill + SkillOrchestrator*